...
export class TokenNode extends Node {
  ...
  value(mult = 1) {
    let sign = this.nodes.reduce((sign, cur) => cur.sign * sign, mult).toString().replace('1', '')
...
export class UMinusTokenNode extends TokenNode {
  ...
  value(mult = 1) {
    let sign = this.nodes.reduce((sign, cur) => cur.sign * sign, mult * -1 * this.node.sign).toString().replace('1', '')
